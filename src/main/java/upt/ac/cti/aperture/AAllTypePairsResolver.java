...
public abstract class AAllTypePairsResolver<J extends IJavaElement> {
	...
	public Set<Pair<IType, IType>> resolve(J javaElement1, J javaElement2) {
		...
		var types1 = (cached1.isPresent() ? cached1.get() : resolve(javaElement1)).parallelStream().filter(t -> {
			try {
		...
		var types2 = (cached2.isPresent() ? cached2.get() : resolve(javaElement2)).parallelStream().filter(t -> {
			try {
				return !t.isAnonymous();
			} catch (JavaModelException e) {
				e.printStackTrace();
