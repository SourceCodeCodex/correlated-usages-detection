...
class ExportInOneJob extends Job {
  ...
  public ExportInOneJob(MWorkingSet mWorkingSet,
      Function<MClass, Double> apertureCoverage,
      CoverageStrategy strategy) {
    super("Export " + mWorkingSet + " (working set) aperture coverage report - " + strategy.name);
  ...
  protected IStatus run(IProgressMonitor monitor) {
    ...
    try {
      ...
      ReportUtil.createReport(mWorkingSet.toString(), coverageStrategy, stream);
